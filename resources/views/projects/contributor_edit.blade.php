...
@section('content')
    ...
    @if (Auth::user()->isAdminProject())
        {!! Form::open(['url' => 'projects/' . Session::get('currentProject')['id'] . '/contributors/' . $contributor->id, 'method' => 'put']) !!}
        {{ 'Name : ' . $contributor->user->name }}
        {{ 'Email : ' . $contributor->user->email }}
        {{ Form::label('role', 'Role') }}
        {{ Form::select('role', $roles, $contributor->role) }}
        {{ Form::submit('Save') }}
        {!! Form::close() !!}
    @endif
        ...
        <div class="modal-dialog modal-dialog-centered" role="document">
            <div class="modal-content">
                ...
                <div class="modal-footer">
                    ...
                    {!! Form::open(['url' => 'projects/' . Session::get('currentProject')['id'] . '/contributors/' . $contributor->id, 'method' => 'delete']) !!}
                    {{ Form::submit('Remove') }}
                    {!! Form::close() !!}
                </div>
            </div>
